...
#### The most important vectors.(Variances)
...
library(factoextra)
pca_data <- prcomp(one_hot_encoded_df,scale = T)
fviz_eig(pca_data,addlabels = T)
```

#### Top 10 the most contributed factors
...
fviz_pca_biplot(pca_data, repel=T,select.var = list(contrib = 10))
```
