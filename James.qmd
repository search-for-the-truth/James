...
rash_df$`Age at diagnosis` <- as.numeric(rash_df$`Age at diagnosis`)
ggplot(rash_df, aes(x = `Age at diagnosis`)) + geom_histogram(bins = 20) + geom_density(kernel = "gaussian", bw= 0.1, alpha=.3)
```

Gender weights

```{r}
ggplot(rash_df, aes(x = "gender")) + geom_bar()
```
